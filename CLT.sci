...
function [T,a] = transf(theta)//in degrees
    m = cosd(theta);
    n = sind(theta);
    R = [[1 0 0];[0 1 0];[0 0 2]];
    T = [[m^2 n^2 2*m*n];..
...
function H = buildH(Qast,thickness,orientation)
    ...
    for k = 1:1:N
        ...
        // H = H - Qast_*thickness(k) // classical theory
...
function [R,imode] = maxStress(FT,FC,sigma)
    ...
    I3 = Macaulay(sigma(3)/FT(3)) + Macaulay(-sigma(3)/FC(3)) + 1.0E-12
...
function [LPF,R1,R6] = TMS(eps_,e1t,e1c,orientation,v12)
    // Truncated Max. Strain (TMS) without matrix cutoff
    // TMS is a laminate f.c. It cannot be called individually per lamina
    LT = sum(orientation);
    NL = length(orientation);
    LPF = 1E12; R1 = LPF; R6 = LPF;//initially set to a huge value
    for k = 1:NL
        eps = rotateEps_(eps_,orientation(k)); //to lamina c.s.
        if eps(1)~=0 then //prevent division by zero
            r1 = Macaulay(e1t/eps(1)) + Macaulay(-e1c/eps(1))//(7.60)
            R1 = min(R1,r1)// tracks min(R1) for all laminas
        end
        if abs(eps(1)-eps(2))~=0 then //prevent division by zero
            r6 = (1 + v12) * max(e1t,e1c) / abs( eps(1) - eps(2) )//(7.60)
            R6 = min(R6,r6)// tracks min(R6) for all laminas
        end
        LPF = min(R1,R6)//tracks overall minimum for all laminas
    end
endfunction
